...
export const getTask = async (userAddress, userProxyAddress, amount, delaySeconds, abis, addresses, provider) => {
...
export const graphQLquery = (address) => {
    return `query { taskReceiptWrappers(where: {user: "${address.toLowerCase()}"}) {
        taskReceipt {
          id
          userProxy
          provider {
