...
`DAPP_FEE_ACTION_ADDR` (address of the fee action you deploy in step 5)

1. Get an [Infura Project ID](https://infura.io) if you don't already have one, then set it to `DAPP_INFURA_ID`
2. Choose an ethereum keypair to be the Gelato Provider. Set the hex private key (with 0x prefix) to `DAPP_PROVIDER_PK`.
...
The script in Step 4 instanciates the provider, deploy the ActionStablecoinFee contract, whitelist the dapp TaskSpec, and provide 2 ETH to the Gelato network.
